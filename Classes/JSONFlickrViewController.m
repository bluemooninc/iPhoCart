...
NSString *const FlickrAPIKey = @"be41b2fcb476c9e0ea8aa3a68a7f824c";
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
  ...
  [photoURLsLargeImage removeAllObjects];
    NSDictionary *jsonArray = [NSDictionary dictionaryWithObject:searchTextField.text forKey:@"search_strings"];
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:jsonArray options:kNilOptions error:nil];
    NSString *encodedString = [jsonData base64EncodedString];
    [self searchFlickrPhotos:encodedString];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data 
{
  ...
  // Build an array from the dictionary for easy access to each entry
	NSArray *photos = [results objectForKey:@"linkUrl"];
	...
	for (NSDictionary *photo in photos)
  {
    ...
    // Notice the "_s" which requests a "small" image 75 x 75 pixels
      NSString *photoURLString = [NSString stringWithFormat:@"http://www.xoopsec.com/uploads/%@",[photo objectForKey:@"image"]];
    ...
    // in on the image if requested
      photoURLString = [NSString stringWithFormat:@"http://www.xoopsec.com/uploads/%@",[photo objectForKey:@"mediumImage"]];
...
-(void)searchFlickrPhotos:(NSString *)text
{
  // Build the string to call the Flickr API
    NSString *urlString = [NSString stringWithFormat:@"https://www.xoopsec.com/modules/bmcart/JsonApi/searchItems/%@", text];
